...
class Solution:
    def duplicateZeros(self, arr: List[int]) -> None:
        ...
        """
        zeroC=0 #Check how many zeroes there all
        kept_length=0 #Get the full length of the array if the  array were combined
        for n in arr:
            if(n==0): zeroC+=1 #Count the number of zeroes
        kept_length=len(arr)+zeroC #Full length of the array
    ...
    # At this point, b is less than a and the loop breaks.
        ...
        a=len(arr)-1 # Length of the first array
        b=kept_length-1 # Length of the full array
        ...
        while(a>=0 & a<b):#Since we are starting from the end, and while the original array length is within the range length of the kept_length array
            if(arr[a]==0): # If a zero is found at position a
                if b<=len(arr)-1: arr[b]=0  # The number at position b is switched to 0 if b (the keptnumbers array length) is within the array size of the original array
                if b-1<=len(arr)-1: arr[b-1]=0 # This is creating the duplicate pair. Again, must the index  be within the original array
                b-=2 # B is decreased by two since the array shift was by 2
            else:
                if b<=len(arr)-1: arr[b]=arr[a] # If the element wasn't 0, then the array at position b is overwritten with the element at position a if b is within the array length 
                b-=1 # Only one shift, so the number is decreased by 1
            a-=1   # Decrease by one as we did the iteration.


#Solution using a double headed queue using python's libraries
class Solution:
    def duplicateZeros(self, arr: List[int]) -> None:
        """
        Do not return anything, modify arr in-place instead.
        """
        # O(N) time and space solution using a queue
        from collections import deque
        
        seen = deque()#Create the empty queue
        
        for n in arr:
            seen.append(n)#Append all the numbers in the array to the queue

        curr = 0 #counter to hold the current index
        
        while curr < len(arr):
            item = seen.popleft() # Pops the first number from the left most position in the queue
            arr[curr] = item #sets the current index  to the number that was popped
            
            if item == 0 and curr != len(arr) - 1: #If the number dequeued was 0 and the current index isn't at the end of the list
                arr[curr + 1] = item # Duplicates zero 
                curr += 1 #Iterates twice  to acknowledge the duplication
            
            curr += 1#Increases the counter holding the index
